...
fn run_specified(names: Vec<String>, force: bool, keep_branch: bool) -> Result<()> {
    ...
    for name in resolved_names {
        ...
        if context.is_main_worktree(&worktree_path) {
            return Err(anyhow!(
                "Cannot remove '{name}': it resolves to the repository's main worktree, \
    ...
    // Workspaces whose working-copy changes survive removal (jj): reported
    // as a note after removal instead of blocking.
    let mut preserved: Vec<(String, Option<String>)> = Vec::new();
    ...
    for (handle, path, branch) in candidates {
        ...
        let status = if path.exists() && !git::has_missing_admin_dir(&path) {
            Some(context.vcs.get_status(&path, None)?)
        } else {
            None
        };

        // Check uncommitted (blocking, except where the backend preserves
        // the changes — see `VcsBackend::preserves_working_copy_on_remove`).
        if status.as_ref().is_some_and(|s| s.is_dirty) {
            if context.vcs.preserves_working_copy_on_remove() {
                preserved.push((handle.clone(), status.and_then(|s| s.head_commit)));
                safe.push(handle);
            } else {
    ...
    for (handle, commit_id) in preserved {
        match commit_id {
            Some(id) if !id.is_empty() => {
                println!(
                    "note: '{handle}' had working-copy changes; jj kept them in commit {}",
                    &id[..id.len().min(12)]
                );
            }
            _ => {
                println!(
                    "note: '{handle}' had working-copy changes; jj kept them as a commit head \
                     (`jj log -r 'heads(all())'`)"
                );
            }
        }
