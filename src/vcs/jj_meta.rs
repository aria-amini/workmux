...
impl WorkmuxMetaStore for JjMetaStore {
    ...
    fn get_all_key(
        ...
        let Ok(path) = metadata_path_for_workdir(workdir) else {
            return values;
        };
        if !path.exists() {
            return values;
        }
        let Ok(_lock) = FileLock::acquire(&lock_path_for(&path)) else {
            return values;
        };
        ...
        let Some(worktree) = doc.get("worktree").and_then(Item::as_table) else {
            return values;
        };
        for (handle, item) in worktree.iter() {
            if let Some(table) = item.as_table()
                && let Some(v) = table.get(key).and_then(Item::as_str)
            {
                values.insert(handle.to_string(), v.to_string());
            }
        }
        values
    }
