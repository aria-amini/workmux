...
//! would break line-oriented parsing. `jj git root` emits one path; only its
//! terminating newline is removed. Another exception is
...
/// unrecorded/stale), the `@` commit id, and the local bookmarks on that
/// workspace's `@`.
///
...
const WORKSPACE_LIST_TEMPLATE: &str = concat!(
    ...
    r#" ++ target.commit_id() ++ "\x1f""#,
    r#" ++ target.local_bookmarks().map(|b| b.name()).join("\x1d") ++ "\x1e""#
);

/// Index of the `@` commit id field in [`WORKSPACE_LIST_TEMPLATE`] output.
const WORKSPACE_LIST_COMMIT_FIELD: usize = 2;
/// Index of the bookmarks field in [`WORKSPACE_LIST_TEMPLATE`] output.
const WORKSPACE_LIST_BOOKMARKS_FIELD: usize = 3;
...
const COMMIT_STATUS_TEMPLATE: &str = concat!(
    ...
    r#" ++ local_bookmarks.map(|b| b.name()).join("\x1d") ++ "\x1f" ++ commit_id ++ "\x1e""#
);
...
/// The youngest local bookmark reachable from `commit_id`'s ancestry, or
/// `None` when no bookmarked ancestor exists.
///
/// After `jj commit`, the workspace's `@` advances to a fresh empty child
/// while the bookmark workmux created stays on the committed work. The
/// workspace list template only sees `@`'s own bookmarks, which would make
/// the workspace look branch-less; this recovers the branch by walking `@`'s
/// ancestry to its youngest bookmarked ancestor. Failures degrade to `None`
/// (same posture as the other best-effort reads here).
fn ancestor_bookmark_of(workdir: Option<&Path>, commit_id: &str) -> Option<String> {
    let revset = format!("heads(::{} & bookmarks())", quote(commit_id));
    let template = r#"local_bookmarks.map(|b| b.name()).join("\x1d") ++ "\x1e""#.to_string();
    let output = query(
        workdir,
    ...
    records(&output)
        .iter()
        .filter_map(|fields| fields.first())
        .flat_map(|field| list_field(field))
        .next()
}
...
impl JjBackend {
    fn git_common_dir(workspace: &Path) -> Result<PathBuf> {
        let output = query(Some(workspace), &["git", "root"])?;
        let path = Path::new(output.strip_suffix('\n').unwrap_or(&output));
        Ok(crate::git::RepositoryIdentity::discover(path)?.common_dir)
    }

    pub(crate) fn colocated_git_identity(
        workspace: &Path,
    ) -> Result<Option<crate::git::RepositoryIdentity>> {
        match std::fs::symlink_metadata(workspace.join(".git")) {
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
            Err(error) => return Err(error.into()),
            Ok(_) => {}
        }
        let identity = crate::git::RepositoryIdentity::discover(workspace)?;
        if identity.common_dir != Self::git_common_dir(workspace)? {
            bail!("Colocated workspace does not belong to the jj repository's Git store");
        }
        if identity.admin_dir == identity.common_dir {
            bail!("Refusing to remove the primary Git worktree registration");
        }
        Ok(Some(identity))
    }
...
impl VcsBackend for JjBackend {
        ...
        Ok(records(&output)
            ...
            .filter_map(|fields| {
                ...
                let bookmarks = fields
                    .get(WORKSPACE_LIST_BOOKMARKS_FIELD)
                    .map(|f| list_field(f))
                    .unwrap_or_default();
                let commit_id = fields
                    .get(WORKSPACE_LIST_COMMIT_FIELD)
                    .cloned()
                    .unwrap_or_default();
                let branch_or_bookmark = bookmarks.into_iter().next().or_else(|| {
                    // After `jj commit`, `@` advances to a fresh empty
                    // child while the bookmark workmux created stays on
                    // the committed work. Recover that branch from the
                    // youngest bookmarked ancestor of `@`.
                    if commit_id.is_empty() {
                        None
                    } else {
                        ancestor_bookmark_of(workdir, &commit_id)
                    }
                });
                Some(WorkspaceEntry {
    ...
    /// Removing a jj workspace never loses working-copy changes: an empty
    /// `@` is abandoned, a non-empty one survives the forget as a head
    /// commit (verified against jj 0.45). Removal flows rely on this to
    /// treat dirty workspaces as informational rather than blocking.
    ...
    fn remove_workspace_at(&self, handle_or_name: &str, common_dir: &Path) -> Result<()> {
        ...
        // "default" is the workspace jj creates at repo init; the user can
        // never create one with that name. Forgetting it orphans the primary
        // checkout, so refuse even under --force.
        if workspace_name == "default" {
            bail!(
                "'{handle_or_name}' resolves to the primary workspace of the jj repository, \
    ...
    fn prune_workspaces_in(&self, common_dir: &Path) -> Result<()> {
        ...
        crate::git::prune_worktrees_in(&Self::git_common_dir(common_dir)?)
            .context("Failed to prune stale colocated Git worktrees")?;
        Ok(())
    }
    ...
    /// The bookmark on this workspace's `@`, or the youngest bookmarked
    /// ancestor, or `None`.
    ///
    /// A branch-less `@` is the *normal* state in jj, so this reports
    /// absence rather than erroring — but after `jj commit` the bookmark
    /// workmux created for the workspace sits one commit below `@`, and
    /// callers ("what branch does this workspace hold?") mean that bookmark.
    /// Walk `@`'s ancestry to recover it, exactly like
    /// [`Self::list_workspaces_in`]'s recovery.
    fn get_current_branch_in(&self, workdir: &Path) -> Result<Option<String>> {
        ...
        let commit_id = fields.get(3).cloned().unwrap_or_default();
        if commit_id.is_empty() {
            return Ok(None);
        }
        Ok(ancestor_bookmark_of(Some(workdir), &commit_id))
    }
    ...
    fn get_status(&self, workspace_path: &Path, main_branch: Option<&str>) -> Result<VcsStatus> {
        ...
        let branch = fields
            ...
            .into_iter()
            .next();
        let head_commit = fields.get(3).filter(|id| !id.is_empty()).cloned();
...
mod tests {
    ...
    fn create_list_remove_round_trip(temp: &Path) {
        ...
        assert!(workspace_path.join(".jj").is_dir());
        assert!(workspace_path.join(".git").is_file());
    ...
    #[test]
    fn create_workspace_colocates_from_primary_and_secondary_despite_config() {
        for temp in [jj_only_fixture(), colocated_fixture()] {
            jj_in(
                temp.path(),
                &["config", "set", "--repo", "git.colocate", "false"],
            );
            let mut source = temp.path().to_path_buf();
            for name in ["first", "second"] {
                ...
                assert_eq!(
                    JjBackend.get_current_branch_in(&path).unwrap().as_deref(),
                    Some(name)
                );
                source = path;
            }
    ...
    fn prune_forgets_workspaces_whose_directory_was_deleted() {
        ...
        backend
            .create_workspace_in(
                &CreateWorkspaceOptions {
                    ...
                    name_or_branch: "live".to_string(),
                    ..opts.clone()
                },
                Some(temp.path()),
            )
            .unwrap();
        let live_identity = JjBackend::colocated_git_identity(&live_path)
            .unwrap()
            .unwrap();
        let removed_identity = JjBackend::colocated_git_identity(&workspace_path)
            .unwrap()
            .unwrap();
        ...
        let common_dir = backend.get_common_dir_in(Some(temp.path())).unwrap();
        backend.prune_workspaces_in(&common_dir).unwrap();
        assert!(!removed_identity.admin_dir.exists());
        assert_eq!(
            JjBackend::colocated_git_identity(&live_path).unwrap(),
            Some(live_identity)
        );
    ...
    #[test]
    fn colocated_git_cleanup_preserves_other_worktrees_and_allows_recreation() {
        for temp in [jj_only_fixture(), colocated_fixture()] {
            let opts = CreateWorkspaceOptions {
                path: temp.path().join("remove"),
                name_or_branch: "remove".to_string(),
                create_branch: true,
                base: Some("main".to_string()),
                track_upstream: false,
            };
            JjBackend
                .create_workspace_in(&opts, Some(temp.path()))
                .unwrap();
            let identity = JjBackend::colocated_git_identity(&opts.path)
                .unwrap()
                .unwrap();
            let other = CreateWorkspaceOptions {
                path: temp.path().join("keep"),
                name_or_branch: "keep".to_string(),
                ..opts.clone()
            };
            JjBackend
                .create_workspace_in(&other, Some(temp.path()))
                .unwrap();
            let other_identity = JjBackend::colocated_git_identity(&other.path)
                .unwrap()
                .unwrap();

            assert!(crate::git::remove_missing_worktree_registration(&identity).is_err());
            assert!(identity.admin_dir.exists());
            JjBackend
                .remove_workspace_at("remove", temp.path())
                .unwrap();
            std::fs::remove_dir_all(&opts.path).unwrap();
            crate::git::remove_missing_worktree_registration(&identity).unwrap();
            assert!(!identity.admin_dir.exists());
            assert_eq!(
                JjBackend::colocated_git_identity(&other.path).unwrap(),
                Some(other_identity)
            );
            crate::git::remove_missing_worktree_registration(&identity).unwrap();
            JjBackend
    ...
    fn create_workspace_without_create_branch_makes_no_bookmark() {
        ...
        // No bookmark was created, but the workspace sits on `main`'s
        // revision, and the ancestor recovery reports that branch — the
        // workspace holds `main`, even though `@` itself carries no
        // bookmark.
        assert_eq!(
            backend.get_current_branch_in(&workspace_path).unwrap(),
            Some("main".to_string())
