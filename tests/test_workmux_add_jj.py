...
@pytest.fixture
def jj_only_repo_path(mux_server: TmuxEnvironment) -> Path:
...
class TestRemoveCleansUpWorkspaceAndMetadata:
    ...
    removes workmux's own per-handle metadata (from `.jj/workmux/metadata.toml`
    ...
    def test_colocated(
        ...
        assert not worktree_path.exists(), f"Worktree {worktree_path} should be removed"
        ...
        metadata_path = jj_colocated_repo_path / ".jj" / "workmux" / "metadata.toml"
    ...
    def test_jj_only(
        ...
        metadata_path = jj_only_repo_path / ".jj" / "workmux" / "metadata.toml"
        assert metadata_path.exists(), "workmux metadata file should still exist"
        metadata = metadata_path.read_text()
        assert f"[worktree.{branch_name}]" not in metadata, (
            f"Metadata for handle {branch_name!r} should have been removed:\n{metadata}"
